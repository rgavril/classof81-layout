	...
	function draw()
	{
		...
		if ( this.is_active ) {
			if ( this.is_gear_selected ) {
				...
				::bottom_text.set("Press any button to start [Title]. Move up or down to select a different game. Move left to change game settings for [Title]. Move righ to access Retro Achivements.");
			}
		}
	}
